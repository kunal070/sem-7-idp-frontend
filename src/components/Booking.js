...
function Booking({ session }) {
  ...
  return (
    <>
    <div style={{ marginLeft: '300px',marginTop : '50px', color: 'black' }}>
      <ActivityCalendar
        theme={explicitTheme}
        blockMargin={4}
        color="#0F3C69"
      ...
      <div style={{ marginLeft: '0px', padding: '100px 50px 0px 350px',color :'black' }}>
      <select
        name="companyType"
      ...
      {selectedOption && (
        <input type="date" min={moment().format("YYYY-MM-DD")} selected={startDate} onChange={handleDateChange}/>
      )}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', padding: '50px 0px 0px 350px',color :'black' }}>
        <div className='width-30'>
          <p className='label' style={{textAlign:'start', paddingRight: "10px"}}>Purpose: </p>
        </div>
        <div className='width-70'>
          <textarea name="purposeResearch" required style={{ backgroundColor: '#eee', width: '317%', minHeight: '20px',borderRadius : 4, color:"#0f3c69" }}/>
          {/* {errors.companyERDAObjective && <span className="error"style={{color: 'red', fontSize: '12px'}}>{errors.companyERDAObjective}</span>} */}
        </div>
      </div>
      <div style={{paddingBottom:20, paddingTop:30, marginInlineStart: '35em'}}>
        <button type="submit" className='savebtn' style={{ borderColor: '#0f3c69', backgroundColor: '#0f3c69', color: 'white', borderRadius: 5, marginInline: 80 }} >Book Laboratory</button>
      </div>
</>
  );
}
